...
namespace Task_1
{
    ...
    {
        public MeleeUnit(int x, int y, /*int health, int maxHealth, int speed, int attack, int attackRange, char symbol,*/ string faction)
        ...
        */
        ...
        public override bool InRange(Unit otherUnit) //to assist in calculating the distance and attacking range relation
        {
            return GetDistance(otherUnit) <= attackRange; //repeats this method, saves need for rewriting copious code
        }

        public override void Kill() //a termination method to change symbol when a death is triggered
        {
            isDestroyed = true;
            isAttacking = false;
            symbol = 'X';
